...
class VisualSimilarityTester:
    ...
    def process_image(self, image_path: str, target_technique: str, viewpoint: str) -> Dict:
        ...
        stick_results = self.pose_analyzer.detect_stick(frame_rgb, [x1, y1, x2, y2])
        
        stick_keypoints = np.zeros((2, 4))
        if stick_results is not None:
            for i, (sx, sy, conf) in enumerate(stick_results[:2]):
                sx_norm = (sx - x1) / w if w > 0 else 0
                sy_norm = (sy - y1) / h if h > 0 else 0
                stick_keypoints[i] = [sx_norm, sy_norm, conf, 0]
        ...
        tips = generate_lesson_tips(low_features, max_tips=3)
