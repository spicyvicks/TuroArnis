...
class PoseAnalyzer:
    def __init__(self, detection_interval=3): 
        ...
        try:
            ...
            print(f"[info] attempting to load classifier wrapper from: {model_path}")
    ...
    def process_frame(self, frame):
        ...
        if pose_results.pose_landmarks and len(tracked_persons) > 0:
            ...
            for person_id, data in analysis_results.items():
                yolo_box = data['bbox']
                iou = self._calculate_iou(mp_box, yolo_box)
                if iou > best_iou:
                    best_iou = iou
            ...
            if best_match_id != -1 and best_iou > 0.3: #30% IoU threshold
                ...
                if self.classifier and live_angles:
                    try:
                        ...
                        prediction_result = self.classifier.predict(live_features_array)
